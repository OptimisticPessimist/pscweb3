...
> 1. **一行セリフ**:  
>    \x60@キャラクター名 セリフ\x60 のように \x60@\x60 で始めると、キャラクターとセリフを1行で書けます。
> 2. **強制シーン見出し**:  
>    \x60.1 第一幕\x60 や \x60.2 第一場\x60 のように \x60.\x60 で始めると、シーン見出しとして認識されます。
> 3. **全角・半角スペースの保持**:  
>    行頭にスペースを入れると、ト書きとして認識され、インデントがそのまま表示されます。
> 4. **あらすじ（Synopsis）の除外**:  
>    「# あらすじ」や「## Synopsis」のように、見出しに特定のキーワードを含めると、そのセクションはシーン番号（1, 2, 3...）として数えられず、香盤表や稽古スケジュール作成画面のリストからも除外されます。


### 📝 脚本情報の入力
アップロード時に以下の情報を設定できます。
- **著作者名**: 脚本の作者名（任意）。
- **公開設定**: 「公開する」にチェックを入れると、他のユーザーが閲覧・インポートできるようになります。
    - ※ 公開すると、プロジェクト作成数制限のカウントから除外されます。
...
1. 脚本詳細ページで **「香盤表」** タブを選択します。
2. 各シーンの行で、登場するキャラクターの出演状態をクリックで切り替えます。
   - **・**（未出演）をクリック → **○**（手動出演）に変更
   - **○**（手動出演）をクリック → **・**（未出演）に変更
   - **●**（セリフあり出演）はクリック不可（脚本から自動生成された出演です）

#### カスタム役の追加
脚本に存在しない役を手動で追加できます。

1. 香盤表の上部にある **「+ 役を追加」** ボタンをクリックします。
2. 役名を入力して追加します。
3. カスタム役はヘッダーがイタリック・オレンジ色で区別されます。
4. 不要になったカスタム役は削除ボタンで削除できます。

#### カスタムシーンの追加
脚本に存在しないシーンを手動で追加できます。

1. 香盤表の下部にある **「+ シーンを追加」** ボタンをクリックします。
2. シーン見出し、幕番号、シーン番号を入力して追加します。
3. カスタムシーンはオレンジ色の背景で区別されます。
4. 不要になったカスタムシーンは削除ボタンで削除できます。

#### シーンの編集
シーンの見出しや番号を変更できます。

1. シーン行の左端にある **鉛筆アイコン** をクリックします。
2. モーダルでシーン見出し、幕番号、シーン番号を編集できます。

### 🔄 脚本のリセット
既存の脚本データを削除して、カスタムデータのみを残すことができます。

1. 脚本メニューから **「脚本リセット」** を選択します。
2. 確認ダイアログが表示されます。
3. リセットすると脚本由来のキャラクター・シーン・マッピングが削除されます。
4. 手動で追加したカスタム役・シーン・手動マッピングは保持されます。
...
> 💡 **脚本なしでのプロジェクト利用**
> プロジェクトは脚本をアップロードしなくても利用できます。カスタム役やカスタムシーンを手動で追加して、香盤表を自由に構成できます。
