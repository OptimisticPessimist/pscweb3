...
async def create_test_project_and_invitation():
    ...
    async with async_session() as session:
        ...
        await session.execute(
            stmt_project,
            {"id": project_id, "owner_id": user_id, "name": "TEST_AUTO_RETRY_PROJECT", "now": now},
        )
        ...
        await session.execute(
            stmt_member,
            {"id": str(uuid.uuid4()), "project_id": project_id, "user_id": user_id, "now": now},
        )
        ...
        print("Project Name: TEST_AUTO_RETRY_PROJECT")
        print(
            f"Invitation URL: https://ashy-river-0227e1e00.3.azurestaticapps.net/invitations/{token}"
        )
        print("--------------------------------------------\n")
