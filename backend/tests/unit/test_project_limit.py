...
from src.db.models import ProjectMember, Script, TheaterProject, User
...
async def test_check_project_limit_excludes_existing_public_projects(db):
    ...
    await db.refresh(
        public_project
    )  # Ensure relationship is loaded if needed, though check_project_limit queries it fresh
...
async def test_check_project_limit_loophole_prevention(db):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await check_project_limit(
            user.id, db, project_id_to_exclude=p_public.id, new_project_is_public=False
        )
    assert excinfo.value.status_code == 400
