...
from src.db.models import SchedulePoll, SchedulePollAnswer, SchedulePollCandidate
...
async def test_poll_upsert_answer(db, test_user):
    ...
    candidate = SchedulePollCandidate(
        poll_id=poll.id, start_datetime=datetime.now(UTC), end_datetime=datetime.now(UTC)
    )
    db.add(candidate)
    await db.commit()
    ...
    res = await db.execute(
        select(SchedulePollAnswer).where(SchedulePollAnswer.candidate_id == candidate.id)
    )
...
async def test_get_unanswered_members(db, test_project, test_user):
    ...
    # Add members
    if not (
        await db.execute(select(ProjectMember).where(ProjectMember.user_id == test_user.id))
    ).scalar_one_or_none():
        db.add(
            ProjectMember(
                project_id=test_project.id, user_id=test_user.id, display_name="User1", role="owner"
            )
        )
    db.add(
        ProjectMember(
    ...
    poll = SchedulePoll(
        id=uuid4(), project_id=test_project.id, title="Test List", creator_id=test_user.id
    )
    db.add(poll)
...
async def test_send_reminder(db, mock_discord_service, test_project, test_user):
    ...
    poll = SchedulePoll(
        id=uuid4(), project_id=test_project.id, title="Reminder Test", creator_id=test_user.id
    )
    db.add(poll)
    await db.flush()
