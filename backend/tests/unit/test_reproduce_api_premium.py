...
from src.api.projects import create_project, ProjectCreate
...
async def test_api_create_premium_project(db: AsyncSession):
    ...
    user = User(discord_id=str(uuid4())[:20], discord_username="premium_tester", premium_password=test_password)
    db.add(user)
    await db.commit()
