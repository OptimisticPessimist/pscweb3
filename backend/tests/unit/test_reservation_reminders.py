...
from src.db.models import Milestone, Reservation, TheaterProject
...
async def milestone_today(db: AsyncSession, project: TheaterProject) -> Milestone:
    ...
    today_utc = today_jst.astimezone(UTC).replace(tzinfo=None)

    ms = Milestone(
        project_id=project.id, title="Today's Event", start_date=today_utc, location="Test Venue"
    )
    db.add(ms)
    await db.commit()
    await db.refresh(ms)
...
@pytest.mark.asyncio
async def test_check_todays_events_basic(
    db: AsyncSession, project: TheaterProject, milestone_today: Milestone
):
    """当日イベントチェック基本テスト."""
