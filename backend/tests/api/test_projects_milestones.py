...
from src.db.models import Milestone, Reservation, TheaterProject, User
from src.dependencies.auth import get_current_user_dep
from src.main import app


@pytest.mark.asyncio
async def test_list_milestones_with_capacity(
    client: AsyncClient, db: AsyncSession, test_project: TheaterProject, test_user: User
):
...
@pytest.mark.asyncio
async def test_update_milestone_capacity(
    client: AsyncClient, db: AsyncSession, test_project: TheaterProject, test_user: User
):
    ms = Milestone(
        project_id=test_project.id,
    ...
    try:
        ...
        response = await client.patch(
            f"/api/projects/{test_project.id}/milestones/{ms.id}", json=payload
        )
