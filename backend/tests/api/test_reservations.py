...
async def cast_member(db: AsyncSession, project: TheaterProject) -> User:
    ...
    script_id = uuid4()
    script = Script(
        id=script_id, project_id=project.id, uploaded_by=user_id, title="Test Script", content=""
    )
    db.add(script)
    await db.flush()
...
@pytest.mark.asyncio
async def test_create_reservation_capacity_error(
    client: AsyncClient, db: AsyncSession, milestone: Milestone
):
    """定員オーバーエラーテスト."""
    # 既に9名予約済み (定員10名)
    r1 = Reservation(milestone_id=milestone.id, name="Existing", email="e@e.com", count=9)
    db.add(r1)
    await db.commit()
...
@pytest.mark.asyncio
async def test_get_cast_members_public(
    client: AsyncClient,
    db: AsyncSession,
...
@pytest.mark.asyncio
async def test_admin_reservation_list(
    client: AsyncClient,
    db: AsyncSession,
    project: TheaterProject,
    milestone: Milestone,
...
async def test_update_attendance(
    client: AsyncClient, db: AsyncSession, milestone: Milestone, project_member: User
):
    """出欠更新テスト."""
    ...
    try:
        response = await client.patch(
            f"/api/reservations/{r.id}/attendance", json={"attended": True}
        )
        assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_public_schedule(
    client: AsyncClient, db: AsyncSession, project: TheaterProject, milestone: Milestone
):
    """公開スケジュール取得テスト."""
