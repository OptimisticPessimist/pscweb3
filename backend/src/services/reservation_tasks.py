...
async def _do_check_todays_events(db: AsyncSession, stats: dict[str, int]) -> dict[str, int]:
    ...
    today_start_utc = today_start_jst.astimezone(UTC).replace(tzinfo=None)
    today_end_utc = today_end_jst.astimezone(UTC).replace(tzinfo=None)

    # 1. 当日のマイルストーンを取得
    stmt = (
        select(Milestone)
        .where(Milestone.start_date >= today_start_utc, Milestone.start_date <= today_end_utc)
        .options(selectinload(Milestone.reservations), selectinload(Milestone.project))
    )
    ...
    for milestone in milestones:
        # 未送信の予約を抽出
        pending_reservations = [r for r in milestone.reservations if r.reminder_sent_at is None]
        ...
        for reservation in pending_reservations:
            try:
                ...
                if sent:
                    # 送信成功時、送信日時を記録
                    reservation.reminder_sent_at = datetime.now(UTC)
                    stats["reminders_sent"] += 1
                    logger.info(
                        f"Reminder sent to {reservation.email} for milestone {milestone.title}"
                    )
                else:
                    stats["errors"] += 1
