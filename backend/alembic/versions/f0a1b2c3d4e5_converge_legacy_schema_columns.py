...
def upgrade() -> None:
    ...
    # theater_projects.is_public: 列が無ければ NOT NULL で追加、
    # nullable で存在する場合（migration_add_project_is_public.sql 由来など）は
    # NULL を false で補完してから NOT NULL 制約を付与する。
    op.execute(
        """
        DO $$
        BEGIN
            IF NOT EXISTS (
                ...
                WHERE table_schema = 'public'
                  AND table_name = 'theater_projects'
            ...
            ELSE
                ...
                UPDATE theater_projects SET is_public = false WHERE is_public IS NULL;
                ALTER TABLE theater_projects
