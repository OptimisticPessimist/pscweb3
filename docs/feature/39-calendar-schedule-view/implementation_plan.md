...
### 3. 役職ベースの必須出席条件（NEW）
演出家や制作担当など、特定の役職（スタッフロール）のメンバーが1人も参加できない日程を「稽古不可」として除外したいという要望に対応します。

**判定手順**:
1. 日程調整の作成時に、「出席必須とする役職」を任意で選択できるようにします。
2. 判定ロジックにおいて、キャストの有無を確認する前に、**「出席必須役職」に属するメンバーが少なくとも1人以上 `ok` または `maybe` 回答をしているか**をチェックします。
3. 必須役職のメンバーが全滅（全員 `ng` または未回答）している候補日程は、全てのシーンについて「稽古不可」とみなします。
