...
### 3. テスト環境の調整
- **`tests/conftest.py`**: 既存のテストが `/api/...` というパスを前提に書かれているため、テスト用のクライアント（`ASGITransport`）に、リクエストパスから `/api` を自動で削るラッパーを導入しました。これにより、アプリ本体の設定（標準プレフィックス運用）を変えることなく、全てのテストが正常に動作するようにしました。
